...
- Paste chooser: cursor-block location, scroll tracking, cancellation and raw URL insertion. Mentions: distinct chip/raw-link presentation, immediate page title/icon updates, legacy migration, Markdown fallback, and searchable labels. File/date references: `@` filtering and keyboard selection, valid ISO dates, upload cancellation/error boundaries, authenticated download, save/reload, exact-bundle preservation, and documented portable Markdown degradation. External preview: mocked OpenGraph title/image, persistence, stale refresh de-duplication, TTL/LRU bounds, hover after reload, SSRF address/URL validation, and inert HTML parsing unit tests.
- Database: typed-model bounds and invalid values; add/rename properties; edit text, number, select, checkbox, and date cells; create/navigate/rename a real child-page row; save/reload; search extraction; exact-bundle ID remapping; and explicit static-table Markdown degradation.
