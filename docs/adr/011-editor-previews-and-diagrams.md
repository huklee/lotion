...
- Use a live custom table-of-contents block and a callout block. The former ordinary-table `/database` baseline is superseded by [ADR-023](023-typed-database-rows.md), which defines the bounded typed-property and child-page-row scope.
...
The `@` suggestion menu now offers a reusable local calendar dialog with month navigation, day buttons and direct date entry. The original plain-text date decision was superseded by [ADR-022](022-file-and-date-references.md), which stores validated typed date atoms. Native dialog semantics provide focus trapping and Escape cancellation. Date reminders and editable date chips remain outside this implementation. TOC buttons explicitly use `justify-content: flex-start` in addition to left text alignment because the generic button style centered flex content.
