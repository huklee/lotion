...
- Checklist completion is available through the checkbox and Ctrl+Enter/⌘Enter. Capture checkbox pointer completion above BlockNote's imperative input event boundary, but defer the state write until the browser finishes its pointer sequence so Firefox does not retry a detached target. Plain-text line insertion inherits the active checklist type so pasting cannot silently turn tasks into paragraphs.
