...
Prepared application checkpoint 0.4.0 adds a browser-local control panel for appearance, editor text size, page width and sidebar startup behavior. Release 0.3.1 fixed single-line inline paste in checklists/callouts and checkbox click scroll stability. Browser tests use isolated per-worker servers/workspaces and full Playwright parallelism. A three-option search plan and the remaining requested incremental work are tracked under `docs/`.

## Project identity

- Current prepared application version: **0.4.0**. Publication commit and CI evidence are recorded after push. The 0.3.1 release commit is `0bdb9b0`; [its GitHub CI](https://github.com/huklee/lotion/actions/runs/34960911713) passed the complete gate on Ubuntu / Node 24. Earlier failure records are historical; read the latest test/history entries before resuming old backlog work.
...
- Follow [version management](versioning.md) and record releases in [changelog](changelog.md). Compatible small feature additions use a minor increment; the prepared release is 0.4.0, not a major increment.
