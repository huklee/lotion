...
The first complete gate passed lint, strict TypeScript/build, and **192/192 unit and integration tests**. Its browser phase passed 177 cases with two intentional platform skips and one WebKit action timing failure. Full-suite traces showed the action button inside a draggable row did not dispatch its click under load. The sidebar actions now separate pointer release from keyboard-synthesized click and stop native row dragging at that boundary. The final create-and-duplicate scenario then passed **10/10** parallel WebKit repetitions. Two intervening full runs found unrelated one-off timeouts in existing new-tab and folder-import cases; their focused three-browser rerun passed **6/6**.

The final standard `npm run check` exited **0** for version 0.22.0: ESLint, strict TypeScript, production build, **192/192 unit and integration tests**, and **178 passed Playwright cases with 2 intentional real-clipboard skips**. The browser phase completed in 4.0 minutes under concurrent local load. The known non-failing large-chunk build warning remains, and user data under `data/` was untouched.
...
The final `CI=1 npm run check` exited **0**: ESLint, strict TypeScript, production build, **176/176 unit and integration tests**, and **156/156 Playwright cases** passed with two workers. The browser phase completed in 2.5 minutes. The known large-chunk warning remains non-failing. User data under `data/` was untouched.
