...
7. [x] Reconstruct enhanced blocks such as bullets, checklists, and headings when structured raw text is copied and pasted as blocks. An empty paragraph recognizes external outlines composed entirely of heading, bullet, numbered-list, or checklist lines. A selection copied with Lotion's block toolbar or `⌘C / Ctrl+C` is separately matched to its validated session record, so mixed block types, nesting, checklist state, and inline styles are restored exactly with fresh IDs into empty or existing content, including after an in-tab reload. `⌘X / Ctrl+X` uses the same safe path and deletes only after the clipboard succeeds. Unrelated prose, HTML, links, and inline Markdown continue through the literal plain-text path.
