...
Established 2026-09-13 from the user's versioning instructions. Read this guide before changing a version or preparing a release. Current release: **0.16.0**, incremented from **0.15.0**.
...
6. Push when requested or already authorized and open one pull request with `main` as the base. Never target an unmerged feature branch or begin the next release from one. Check the GitHub Actions run for that commit, fix failures, and repeat the applicable checks before declaring the release verified.
7. Merge the successful pull request into `main`, fetch `origin/main`, and verify the release commit is integrated before starting the next fix or feature. Distinguish a successful local check, push, pull request, remote CI, and merge; none implies the next stage by itself.
8. A Git tag or GitHub release is a separate publication artifact. Create `vX.Y.Z` on the tested release commit when that workflow is requested/authorized; do not claim a tag exists just because package metadata was updated. Never move or overwrite a published tag to conceal later corrections.
